...
const buttonClass =
  "btn btn-circle border-1 border-white text-white hover:bg-white hover:text-black active:bg-white active:text-black bg-opacity-20 sm:bg-opacity-0 hidden sm:flex shadow-lg border border-base-200 group-hover:bg-white group-hover:text-black";
const mobileButtonClass =
  "btn rounded-full dark:text-black dark:hover:text-white sm:hidden rounded-full -py-2 w-10 h-10  shadow-lg border border-base-200";
...
const DefaultCaresoulSmooth = ({ children }: { children?: ReactNode[] }) => {
  ...
  return (
    <>
      <div
        className="group carousel bg-base-100 w-full min-h-screen"
        style={{ top: "0", left: "0" }}
        ref={sliderRef}
      ...
      <div
        className="flex carousel-indicators gap-2"
        style={{
          zIndex: 50,
          position: "relative",
          left: "0",
      ...
      >
        {children?.map((_, index) => (
          <a
            className={`${mobileButtonClass} ${index === active ? "active bg-primary" : "bg-gray-300"} rounded-full overflow-text`}
