...
const CookieConsent = () => {
    ...
    return (
        <div className={`fixed bottom-0 left-0 right-0 text-base-content px-24 pb-2 select-none ${show ? 'block' : 'hidden'}`} style={{ zIndex: 1000 }} id="cookie-popup">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-base-300 text-base-content shadow-md rounded-lg max-w-4xl mx-auto border border-base-100">
                ...
                <div className="text-right items-center text-center md:text-right">
                    <button className="btn btn-sm btn-ghost" onClick={() => setShow(false)}>{t('COOKIES.DISAGREE')}</button>
                    <button className="btn btn-sm btn-primary ml-2" onClick={handleAccept}>{t('COOKIES.AGREE')}</button>
                </div>
            </div>
        </div>
    );
