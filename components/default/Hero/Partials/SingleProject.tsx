        ...
        <article className={`p-6 rounded-lg border shadow-md ${project.bgColor} ${project.textColor}`}>
            <div className="flex items-center mb-5 text-black">
                {project.tags.map((tag) => (
                    ...
                    >
                        <FontAwesomeIcon icon={tag.icon} style={{ width: '1rem', height: '1rem', marginRight: '0.25rem', paddingTop: '0.25rem' }} />
            ...
            <div className="flex justify-between items-center">
                <div className="flex items-center space-x-4">
                    {project.urls.map((url) => (
                        <a key={url.url} href={url.url} className="inline-flex items-center font-medium hover:underline">
                            <FontAwesomeIcon
                                icon={url.icon ? url.icon : url.url.startsWith("https://github.com") ? faGithub : faGlobe}
                                style={{ width: '1rem', height: '1rem', marginRight: '0.25rem', paddingTop: '0.25rem' }}
                            />
                            {url.title ? url.title : url.type === 'GitHub' ? 'GitHub' : 'Demo'}
                        </a>
                    ))}
                </div>
            </div>
