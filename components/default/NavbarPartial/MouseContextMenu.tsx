...
const MouseContextMenu = ({ showContextMenu }: { showContextMenu: boolean }) => {
    ...
    const [cookies, setCookie, removeCookie] = useCookies(['user', 'language', 'theme']);
    ...
    return (
        <>
            {showContextMenu && (
                <div className="absolute z-10 hidden md:block bg-base-100 shadow-lg rounded-lg" style={{ top: position.y, left: position.x }}>
                    <div className="bg-base-100 shadow-lg rounded-lg">
                        <div className="p-2">
                            <p className="text-sm">stats</p>
                            <p className="text-sm">Ent: {timeUserEntered.toLocaleTimeString()}</p>
                            <p className="text-sm">Still: {timeDifferenceText}</p>
                            <p className="text-sm">user: {cookies.user}</p>
                            <p className="text-sm">lang: {cookies.language || localStorage.getItem('language')}</p>
                            <p className="text-sm">theme: {cookies.theme || localStorage.getItem('theme')}</p>
                        </div>
                    </div>
                </div>
            )}
